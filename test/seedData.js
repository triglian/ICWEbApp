...
program.events.forEach(function (e) {
  ...
  if(!e.speaker.ref) {
    ...
    e.speaker = speaker._id

  } else {
    for (var i = 0 ; i < speakers.data.length;i++){
      if(e.speaker.ref == speakers.data[i].name){
        e.speaker = speakers.data[i]._id
        speakers.data[i].events.push(e._id);
      }
    }
  }
  events.data.push(e)
